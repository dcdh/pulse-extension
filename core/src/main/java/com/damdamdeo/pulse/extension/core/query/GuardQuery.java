...
public final class GuardQuery<I, P extends Projection, R extends Result<P>> implements Query<I, P, R> {
    ...
    private final Query<I, P, R> decorated;
    ...
    public GuardQuery(final ExecutionContextProvider executionContextProvider,
        ...
        this.participantsProvider = Objects.requireNonNull(participantsProvider);
    ...
    public R execute(final I input) throws QueryException {
        ...
        for (final Audience audience : audiences) {
            final R result = switch (audience) {
                ...
                case PARTICIPANT -> {
                    final R executed = decorated.execute(input);
                    final Set<ExecutedBy> executedByEligibles = participantsProvider.findParticipants(executed.aggregateIds());
