...
class CheckerTest {
    ...
    void shouldFailWhenTodoIsNotImportant() {
        ...
        assertAll(
                () -> assertThatThrownBy(() -> IMPORTANT_NEXT_IN_PROGRESS.check(givenTodo, EXECUTION_CONTEXT))
                        ...
                        .hasRootCauseMessage("la todo Damien-000001 doit être importante"),
    ...
    void shouldFailWhenTodoIsImportantAndNotInProgress() {
        // Given
        final Todo givenTodo = new Todo(new TodoId("Damien", TodoId.SEQUENCE_NUMBER_1), "lorem", Status.DONE, true);
        ...
        assertAll(
                () -> assertThatThrownBy(() -> IMPORTANT_NEXT_IN_PROGRESS.check(givenTodo, EXECUTION_CONTEXT))
                        .isInstanceOf(BusinessException.class)
                        .hasRootCauseInstanceOf(IllegalStateException.class)
                        .hasRootCauseMessage("la todo Damien-000001 doit être in progress"),
                () -> assertThat(IMPORTANT_NEXT_IN_PROGRESS.isSatisfiedBy(givenTodo, EXECUTION_CONTEXT)).isFalse());
    }
}
