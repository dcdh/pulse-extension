...
import com.damdamdeo.pulse.extension.query.runtime.CachedParticipantsProvider;
...
public class CachingProcessor {
    ...
    private static final Map<String, String> CACHING_CONFIGURATIONS = Map.of(
            "quarkus.cache.caffeine.\"participant\".initial-capacity", "10000",
            "quarkus.cache.caffeine.\"participant\".maximum-size", "10000",
            "quarkus.cache.caffeine.\"participant\".expire-after-write", "7D");
    ...
    List<AdditionalIndexedClassesBuildItem> additionalIndexedClassesBuildItem(final Capabilities capabilities) {
        if (capabilities.isPresent(Capability.CACHE)) {
            return List.of(new AdditionalIndexedClassesBuildItem(CachedParticipantsProvider.class.getName()));
        } else {
            return List.of();
        }
    }
    ...
    List<AdditionalBeanBuildItem> additionalBeanBuildItems(final Capabilities capabilities) {
        if (capabilities.isPresent(Capability.CACHE)) {
            return List.of(AdditionalBeanBuildItem.builder().addBeanClass(CachedParticipantsProvider.class).build());
        } else {
            return List.of();
        }
    }
    ...
    List<ContentBuildItem> contentBuildItems(final Capabilities capabilities) {
        if (capabilities.isPresent(Capability.CACHE)) {
            ...
            contentBuildItems.add(new ContentBuildItem(new Title(Title.Level.SECOND, "Participant caching configuration")));
