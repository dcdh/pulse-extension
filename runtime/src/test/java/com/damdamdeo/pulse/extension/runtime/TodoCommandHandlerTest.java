...
public class TodoCommandHandlerTest {
    ...
    void shouldCreateTodo() {
        // Given
        final CreateTodo createTodo = new CreateTodo(new TodoId(new UUID(0, 0)), "lorem ipsum");
        doReturn(List.of()).when(eventRepository).loadOrderByVersionASC(Todo.class, new TodoId(new UUID(0, 0)));
        ...
        // Then
        assertAll(
                () -> assertThat(todoCreated.id()).isEqualTo(new TodoId(new UUID(0, 0))),
                ...
                () -> verify(eventRepository, times(1)).save(
                        Todo.class, List.of(new VersionizedEvent<>(
    ...
    void shouldMarkTodoAsDone() {
        // Given
        final MarkTodoAsDone markTodoAsDone = new MarkTodoAsDone(new TodoId(new UUID(0, 0)));
        doReturn(List.of(new NewTodoCreated(new TodoId(new UUID(0, 0)), "lorem ipsum")))
                .when(eventRepository).loadOrderByVersionASC(Todo.class, new TodoId(new UUID(0, 0)));
        ...
        // Then
        assertAll(
                () -> assertThat(todoMarkedAsDone.id()).isEqualTo(new TodoId(new UUID(0, 0))),
