...
class JdbcPostgresEncodedDetailedInvolvedRepositoryTest {
    ...
    void shouldFindByAggregateId() throws TraceRepositoryException {
        ...
        // When
        executions.add(jdbcPostgresEncodedDetailedInvolvedRepository.findBy(TodoId.USER_1_TODO_1, new Pagination(0, 1)));
        executions.add(jdbcPostgresEncodedDetailedInvolvedRepository.findBy(TodoId.USER_1_TODO_1, new Pagination(1, 1)));
        executions.add(jdbcPostgresEncodedDetailedInvolvedRepository.findBy(TodoId.USER_1_TODO_1, new Pagination(2, 1)));
        executions.add(jdbcPostgresEncodedDetailedInvolvedRepository.findBy(TodoId.USER_1_TODO_1, new Pagination(0, 10)));

        // Then
        assertThat(executions).containsExactly(
                new Page<>(List.of(
                        new EncodedDetailedInvolved(new TraceId(1L), new EncodedInvolved(TodoId.USER_1_TODO_1, new ExecutedByHashed("EU:alice-hashed"), new ExecutedByEncoded("EU:aliceEncoded")),
                                new From("from"), new ExecutedAt(Instant.parse("2026-09-06T12:00:00Z")))),
                        new Pagination(0, 1), 2L),
                new Page<>(List.of(
                        new EncodedDetailedInvolved(new TraceId(1L), new EncodedInvolved(TodoId.USER_1_TODO_1, new ExecutedByHashed("EU:bob-hashed"), new ExecutedByEncoded("EU:bobEncoded")),
                                new From("from"), new ExecutedAt(Instant.parse("2026-09-06T12:00:00Z")))),
